...
- Misused `.slice(250)` thinking it would keep "the first 250 characters" — it actually does the opposite, returning everything _after_ index 250, which is empty or near-empty for short text. The correct range for "first 250 characters" would have been `.slice(0, 250)`
...
- `new Date()` must be created _inside_ `setInterval`'s callback, not outside it — otherwise it captures the time once at page load and never updates, since `setInterval` only re-runs the function body, not the variable declarations outside it
...
- AM/PM must be decided using the _original_ 24-hour value, before converting it to 12-hour format — deciding it after conversion would lose the information needed to tell AM from PM
...
- Displayed the hour using its raw 24-hour value before converting it to 12-hour format, because the conversion logic was written _after_ the line that already set `hour.textContent` — order of operations mattered here
...
## Task 10 — Dark / Light Theme Toggle

A theme toggle button that switches the page between dark and light mode,
built with TypeScript.

What I built:

- A card with title, description, and a toggle button
- Clicking the button switches all colors (background, card, text, button)
  between dark and light themes
- Button label updates to show what the next action will be
  ("☀️ Light Mode" when dark, "🌙 Dark Mode" when light)
- Default theme is dark on page load

Things I learned:

- A boolean toggle variable (`isDark`) must be declared outside the click
  listener with `let` (not `const`) — inside the listener it resets every
  click, so state is never remembered between clicks
- `isDark = !isDark` is the cleanest way to flip a boolean — no if/else
  needed just for the toggle itself
- `style.backgroundColor`, `style.color`, `style.borderColor` are the
  TypeScript/JS properties for inline styles — CSS property names become
  camelCase (e.g. `background-color` → `backgroundColor`)
- Each element that needs to change theme needs its own variable and its
  own style updates — forgetting one element leaves it stuck in the old theme
- `let isDark: boolean = true` sets the initial state as dark, which must
  match the actual colors set in HTML/Tailwind — if they don't match, the
  first click goes in the wrong direction

Mistakes I made:

- Put `isDark` inside the click listener — it reset to `false` on every
  click so the toggle never actually switched states
- Only updated the button text, forgot to update body/card/title/desc colors
- Used `const` instead of `let` for `isDark` — `const` can't be reassigned,
  so `isDark = !isDark` would throw an error

---

_More tasks coming daily._
